            ...
            if IsValid(ent) then
                ...
                if not indinfo.HideHUD and indinfo.Ready then
                    ...
                    elseif (indinfo.Style == 2) then
                        ...
                        -- Draw only if we have a factor
                        if (indinfo.Factor > 0) then
...
local function HUDIndicatorRegister(um)
    ...
    -- First-time register
    if not hudindicators[eindex].Factor then
...
local function HUDIndicatorStylePercent(um)
    ...
    hudindicators[eindex].AColor = {
        r = ainfo[1],
        g = ainfo[2],
        b = ainfo[3]
    }

    hudindicators[eindex].BColor = {
        r = binfo[1],
        g = binfo[2],
        b = binfo[3]
    }
