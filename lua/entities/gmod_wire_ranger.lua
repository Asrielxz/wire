...
function ENT:Setup(range, default_zero, show_beam, ignore_world, trace_water, out_dist, out_pos, out_vel, out_ang, out_col, out_val, out_sid, out_uid, out_eid, out_hnrm, hiRes)
    ...
    if range then
        self:SetBeamLength(math.min(range, 64000))
    end

    if show_beam ~= nil then
...
function ENT:ShowOutput(dist, pos, vel, ang, col, val, sid, uid, ent, hnrm, trace)
    ...
    if (self.out_pos) then
        txt = txt .. string.format("\nPosition = %s, %s, %s", round(pos.x, 3), round(pos.y, 3), round(pos.z, 3))
    end

    if (self.out_vel) then
        txt = txt .. string.format("\nVelocity = %s, %s, %s", round(vel.x, 3), round(vel.y, 3), round(vel.z, 3))
    end

    if (self.out_ang) then
        txt = txt .. string.format("\nAngles = %s, %s, %s", round(ang.pitch, 3), round(ang.yaw, 3), round(ang.roll, 3))
    end

    if (self.out_col) then
        txt = txt .. string.format("\nColor = %s, %s, %s, %s", round(col.r), round(col.g), round(col.b), round(col.a))
    end

    if (self.out_val) then
        txt = txt .. string.format("\nValue = %s ValSize = %s", round(self.Outputs["Val"].Value or 0, 3), #(val or {}))
    end
    ...
    if (self.out_hnrm) then
        txt = txt .. string.format("\nHitNormal = %s, %s, %s", round(hnrm.x, 3), round(hnrm.y, 3), round(hnrm.z, 3))
    end
