...
function ENT:CalcOutput(iter)
	if (self.Action) and (self.Action.output) then
		if (self.Action.outputs) then
			...
			if WireLib.RateLimit:Limit(WireLib.GetOwner(self)) then return end
...
function ENT:ShowOutput()
	...
	if (self.Action) then
		...
		if (self.Action.label) then
			txt = txt .. "\n" .. self.Action.label(self:GetActionOutputs(), unpack(self:GetActionInputs(true), 1, #self.Action.inputs))
		end
	else
		txt = "Invalid gate!"
	end
