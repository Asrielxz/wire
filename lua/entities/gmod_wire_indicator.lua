...
if CLIENT then
    ...
    local function drawColorSlider(x, y, w, h, self)
        ...
        if self.a == self.b then
            draw.DrawText("Can't draw color bar because A == B", "GModWorldtip", x + w / 2, y + h / 2, white, TEXT_ALIGN_CENTER)
        ...
        if self.value == self.b then
            find_selected = self.b
        end
...
function MakeWire7Seg(pl, Pos, Ang, Model, a, ar, ag, ab, aa, b, br, bg, bb, ba)
    ...
    local prototypes = {{"G", Angle(0, 0, 0), Vector(0, 0)}, {"A", Angle(0, 0, 0), Vector(0, 2)}, {"B", Angle(0, 0, 90), Vector(1, 1)}, {"C", Angle(0, 0, 90), Vector(1, -1)}, {"D", Angle(0, 0, 0), Vector(0, -2)}, {"E", Angle(0, 0, 90), Vector(-1, -1)}, {"F", Angle(0, 0, 90), Vector(-1, 1)}}
