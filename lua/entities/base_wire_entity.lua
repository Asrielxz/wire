...
if CLIENT then
    ...
    function ENT:GetWorldTipPositions(w, h, w_body, h_body, w_footer, h_footer)
        ...
        -- if the position is right in your face, get a better position
        if pos == spos then
    ...
    function ENT:DrawWorldTipOutline(pos)
        ...
        local poly = {
            ...
            {
                x = pos.max.x,
                y = pos.max.y - edgesize + 0.5,
                u = 0,
                v = 0
            },
            {
                x = pos.max.x - edgesize + 0.5,
                y = pos.max.y,
                u = 0,
                v = 0
            },
    ...
    function ENT:DrawWorldTip()
        ...
        if h_body == 0 then
            h_total = h_total - h_body - edgesize
        end

        local pos = self:GetWorldTipPositions(w_total + edgesize * 2, h_total + edgesize, w_body, h_body, w_footer, h_footer)
...
function ENT:SetOverlayText(txt)
    ...
    if not self.OverlayData_UpdateTime then
        self.OverlayData_UpdateTime = {}
    end
...
timer.Create("WireOverlayUpdate", 0.1, 0, function()
    for _, ply in ipairs(player.GetAll()) do
        ...
        if IsValid(ent) and ent.IsWire and ent.OverlayData and ent.OverlayData_UpdateTime and ent.OverlayData_UpdateTime.time > (ent.OverlayData_UpdateTime[ply] or 0) then
