...
function ENT:SetupDataTables()
    self:NetworkVar("Bool", 0, "DrawBackground", {
        KeyName = "DrawBackground",
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_graphics_tablet_mode",
            order = 1
        }
    })

    self:NetworkVar("Bool", 1, "CursorMode", {
        KeyName = "CursorMode",
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_graphics_tablet_draw_background",
            order = 2
        }
    })
end
...
function ENT:Setup(gmode, draw_background)
    if gmode ~= nil then
        self:SetCursorMode(gmode)
    end

    if draw_background ~= nil then
