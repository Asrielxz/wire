...
function ENT:SetupDataTables()
    self:NetworkVar("Bool", 0, "SingleValue", {
        ...
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_screen_singlevalue",
            order = 1
        }
    })

    self:NetworkVar("Bool", 1, "SingleBigFont", {
        KeyName = "SingleBigFont",
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_screen_singlebigfont",
            order = 2
        }
    })
    ...
    self:NetworkVar("Bool", 2, "LeftAlign", {
        ...
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_screen_leftalign",
            order = 3
        }
    })
    ...
    self:NetworkVar("Bool", 3, "Floor", {
        ...
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_screen_floor",
            order = 4
        }
    })

    self:NetworkVar("Bool", 4, "FormatNumber", {
        KeyName = "FormatNumber",
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_screen_formatnumber",
            order = 5
        }
    })
    ...
    self:NetworkVar("Bool", 5, "FormatTime", {
        ...
        Edit = {
            type = "Boolean",
            title = "#Tool_wire_screen_formattime",
            order = 6
        }
    })
...
if CLIENT then
    ...
    function ENT:DrawNumber(header, value, x, y, w, h)
        ...
        -- format as time, aka duration - override formatnumber and floor settings
        if self:GetFormatTime() then
...
function ENT:Setup(SingleValue, SingleBigFont, TextA, TextB, LeftAlign, Floor, FormatNumber, FormatTime)
    ...
    if SingleBigFont ~= nil then
        self:SetSingleBigFont(SingleBigFont)
    end

    if LeftAlign ~= nil then
    ...
    if FormatNumber ~= nil then
        self:SetFormatNumber(FormatNumber)
    end

    if FormatTime ~= nil then
