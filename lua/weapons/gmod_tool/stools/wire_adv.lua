    ...
    TOOL.Information = {
        {
            name = "left_0",
            stage = 0,
            text = "Select input (Shift: Select multiple; Alt: Select all)"
        },
        {
            name = "right_0",
            stage = 0,
        ...
        },
        {
            name = "reload_0",
            stage = 0,
            text = "Unlink (Alt: Unlink all)"
        },
        {
            name = "mwheel_0",
            stage = 0,
            text = "Mouse wheel: Next"
        },
        {
            name = "left_1",
            stage = 1,
        ...
        },
        {
            name = "left_2",
            stage = 2,
            text = "Select output (Alt: Auto-connect matching input/outputs)"
        },
        {
            name = "right_2",
            stage = 2,
...
if SERVER then
    ...
    net.Receive("wire_adv_upload", function(len, ply)
        ...
        for i = 1, #wirings do
            ...
            if IsValid(inputentity) and IsValid(outputentity) and hook.Run("CanTool", ply, WireLib.dummytrace(inputentity), "wire_adv") and hook.Run("CanTool", ply, WireLib.dummytrace(outputentity), "wire_adv") then
...
elseif CLIENT then
    ...
    -- Garry's stage functions didn't work, had to make my own
    ...
    -- Removes any wirings that are no longer valid
    function TOOL:SanitizeUpload()
        for i = #self.Wiring, 1, -1 do
            ...
            if not IsValid(inputentity) or not IsValid(outputentity) or not outputname then
                -- we don't need to check everything because only these things can possibly be invalid
    ...
    function TOOL:CachePorts(ent)
        ...
        if self.ShowEntity then
            if outputs then
                ...
                for i = 1, #outputs do
                    if (outputs[i][1] == "entity" and outputs[i][2] == "ENTITY") or (outputs[i][1] == "Create Entity" and outputs[i][2] == "ENTITY") then
                        found = true
                        break
                    end
                end
                ...
                if not found then
                    -- we don't want to copy it twice
                    if not copied then
    ...
    function TOOL:WireStart(entity, pos, inputname, inputtype)
        ...
        -- wiring is already started, user wants to cancel it
        if wiring then
    ...
    function TOOL:AutoWiringTypeLookup(ent)
        if not IsValid(ent) then
            ...
            for i = 1, #outputs do
                ...
                -- if we haven't found any outputs of this type yet,
                if self.AutoWiringTypeLookup_t[outputtype] == nil then
                    self.AutoWiringTypeLookup_t[outputtype] = i -- set the index
                elseif self.AutoWiringTypeLookup_t[outputtype] ~= nil then
                    -- if we've already found outputs of this type,
    ...
    function TOOL:UpdateTraceForSurface(trace, parent)
        ...
        local traceVectors = {entityUp, entityForward, entityRight, -entityUp, -entityForward, -entityRight}
        ...
        if not foundParent then
            ...
            if not IsValid(parent) or parent == game.GetWorld() then return end -- Hit the world, don't update the trace.
    ...
    function TOOL:LeftClick(trace)
        ...
        if IsValid(trace.Entity) then
            ...
            elseif self:GetStage() == 1 then
                ...
                -- there's only one element in the table
                if next(outputs, next(outputs)) == nil then
                    self.wtfgarry = 0
        ...
        if self:GetStage() == 2 then
            ...
            if alt then
                ...
                for i = 1, #self.Wiring do
                    ...
                    -- if we didn't find a matching name & type, check if there's only one matching type (ignoring name)
                    if not found then
    ...
    -- wtfgarry (have to have a delay in single player or the hook won't get added)
    ...
    function TOOL:LoadMemorizedIndex(ent, forceload)
        if ent ~= self.AimingEnt or self:GetStage() ~= self.AimingStage or forceload then
            if self:GetStage() == 2 and self.CurrentEntity ~= ent then return end -- if you aim away during stage 2, don't change CurrentWireIndex
            ...
            if IsValid(ent) then
                ...
                elseif self.WireIndexMemory[ent:GetClass()] and self.WireIndexMemory[ent:GetClass()][self:GetStage()] then
                    -- if this entity doesn't have a stored position, get it from the class instead
    ...
    function TOOL:IsHighlighted(name, tbl, ent, idx)
        ...
        if name == "Outputs" and self:GetStage() == 2 then
            -- if we're holding alt, highlight all outputs that will be wired to
            if alt then
                ...
                for i = 1, #self.WiringRender do
                    ...
                    if self:IsMatch(inputname, inputtype, outputname, outputtype, true) then return true end
                    local _idx = self:AutoWiringTypeLookup_Check(inputtype)

                    if _idx then
                        local _outputtype = tbl[_idx][2]
                        if outputtype == _outputtype then return true end
                    end
                end
            else
        ...
        elseif name == "Selected" and self:GetStage() == 2 and alt then
            -- highlight all selected inputs that will be wired
            local inputs, outputs = self:GetPorts(ent)
    ...
    function TOOL:IsBlocked(name, tbl, ent, idx)
        -- Gray out the ones that we can't wire any of the selected inputs to
        if name == "Outputs" and self:GetStage() > 0 then
            for i = 1, #self.WiringRender do
                local inputtype = self.WiringRender[i][2]
                if tbl[idx][2] == inputtype then return false end
            end

            return true
        elseif name == "Selected" and self:GetStage() == 2 then
            -- Gray out the ones that won't be able to be wired to any input
            if self:GetOwner():KeyDown(IN_WALK) then
                local inputs, outputs = self:GetPorts(ent)
    ...
    function TOOL:DrawList(name, tbl, ent, x, y, w, h, fonth)
        ...
        for i = 1, #tbl do
            ...
            if highlighted then
                ...
                if diffcolor and (self.CurrentWireIndex == i or self:GetOwner():KeyDown(IN_WALK)) then
                    clr = Color(100, 100, 175, 192)
                elseif diffcolor then
                    clr = Color(0, 0, 150, 192)
                end
    ...
    function TOOL:BeginRenderingCurrentWire()
        ...
        hook.Add("PostDrawOpaqueRenderables", "Wire.ToolWireRenderHook", function()
            ...
            for i = 1, #self.Wiring do
                ...
                if not theEnt:IsValid() then break end
    ...
    function TOOL:IsMatch(inputname, inputtype, outputname, outputtype, checkcreated)
        if checkcreated then
            return (outputname == inputname and outputtype == inputtype) or (inputtype == "WIRELINK" and (outputname == "wirelink" or outputname == "Create Wirelink") and outputtype == "WIRELINK") or (inputtype == "ENTITY" and (outputname == "entity" or outputname == "Create Entity") and outputtype == "ENTITY")
        else
    ...
    function TOOL:WiringRenderFind(inputname, inputtype)
        for i = 1, #self.WiringRender do
            ...
            if self:IsMatch(wiringrender[1], wiringrender[2], inputname, inputtype) then return wiringrender, i end
        end
    end
    ...
    function TOOL.BuildCPanel(panel)
        panel:AddControl("Header", {
            Text = "#Tool.wire_adv.name",
            Description = "#Tool.wire_adv.desc2"
        })

        WireToolHelpers.MakePresetControl(panel, "wire_adv")
        panel:NumSlider("#WireTool_width", "wire_adv_width", 0, 5, 2)

        local matselect = panel:AddControl("RopeMaterial", {
            Label = "#WireTool_material",
            convar = "wire_adv_material"
        })
