...
GateActions["vector_add"] = {
	...
	label = function(Out, ...)
		...
		return format("%s = (%d,%d,%d)", sub(tip, 3), Out.x, Out.y, Out.z)
	end
}
...
GateActions["vector_sub"] = {
	...
	label = function(Out, A, B) return format("%s - %s = (%d,%d,%d)", A, B, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_neg"] = {
	...
	label = function(Out, A) return format("-%s = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_dot"] = {
	...
	label = function(Out, A, B) return format("dot(%s, %s) = %d", A, B, Out) end
}
...
GateActions["vector_cross"] = {
	...
	label = function(Out, A, B) return format("cross(%s, %s) = (%d,%d,%d)", A, B, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_ang"] = {
	...
	label = function(Out, A) return format("ang(%s) = %d, %d", A, Out.Yaw, Out.Pitch) end
}
...
GateActions["vector_angrad"] = {
	...
	output = function(gate, A)
		...
		return (ang.y * pi / 180), (ang.p * pi / 180)
	end,
	label = function(Out, A) return format("angr(%s) = %d, %d", A, Out.Yaw, Out.Pitch) end
}
...
GateActions["vector_mag"] = {
	...
	label = function(Out, A) return format("|%s| = %d", A, Out) end
}
...
GateActions["vector_convto"] = {
	...
	outputtypes = {"VECTOR"},
	output = function(gate, X, Y, Z) return Vector(X, Y, Z) end,
	label = function(Out, X, Y, Z) return format("vector(%s,%s,%s) = (%d,%d,%d)", X, Y, Z, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_convfrom"] = {
	...
	output = function(gate, A)
		if (A and isvector(A)) then return A.x, A.y, A.z end

		return 0, 0, 0
	end,
	label = function(Out, A) return format("%s -> X:%d Y:%d Z:%d", A, Out.X, Out.Y, Out.Z) end
}
...
GateActions["vector_norm"] = {
	...
	label = function(Out, A) return format("norm(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}

--return "norm(" .. A .. ") = [" .. Round(Out.x,3) .. "," .. Round(Out.y,3) .. "," .. Round(Out.z,3) .. "]"
-- Identity
...
GateActions["vector_rand"] = {
	...
	timed = true,
	output = function(gate)
		local vec = Vector(random(), random(), random())
		vec:Normalize()

		return vec
	end,
	label = function(Out) return "Random Vector" end
}
...
GateActions["vector_derive"] = {
	...
	reset = function(gate)
		gate.LastT, gate.LastA = CurTime(), vector_origin
	end,
	label = function(Out, A) return format("diff(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_cint"] = {
	...
	output = function(gate, A)
		...
		gate.LastT, gate.Integral = t, (gate.Integral or vector_origin) + A * dT
	...
	reset = function(gate)
		gate.Integral, gate.LastT = vector_origin, CurTime()
	end,
	label = function(Out, A) return format("int(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_mux"] = {
	...
	output = function(gate, Sel, ...)
		-- If Sel is unwired, because of compact_inputs 3, it will become the first vector input, so just return Vector(0,0,0)
		if isnumber(Sel) then
			Sel = floor(Sel)
			if Sel > 0 and Sel <= 8 then return ({...})[Sel] end
		end

		return vector_origin
	end,
	label = function(Out, Sel, ...) return format("Select: %s  Out: (%d,%d,%d)", Sel, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_dmx"] = {
	...
	output = function(gate, Sel, In)
		local Out = {vector_origin, vector_origin, vector_origin, vector_origin, vector_origin, vector_origin, vector_origin, vector_origin}
		Sel = floor(Sel)
	...
	label = function(Out, Sel, In)
		...
		return format("Select: %s, In: (%d,%d,%d)", Sel, In.x, In.y, In.z)
	end
}
...
GateActions["vector_latch"] = {
	...
	output = function(gate, In, Clk)
		...
		return gate.LatchStore or vector_origin
	end,
	reset = function(gate)
		gate.LatchStore = vector_origin
		gate.PrevValue = 0
	end,
	label = function(Out, In, Clk) return format("Latch Data: %s  Clock: %s  Out: (%d,%d,%d)", In, Clk, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_positive"] = {
	...
	label = function(Out, A) return format("abs(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_round"] = {
	...
	output = function(gate, A)
		...
		return Vector(Round(A.x), Round(A.y), Round(A.z))
	end,
	label = function(Out, A) return format("round(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_max"] = {
	...
	label = function(Out, A, B) return format("max(%s , %s) = (%d,%d,%d)", A, B, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_min"] = {
	...
	label = function(Out, A, B) return format("min(%s , %s) = (%d,%d,%d)", A, B, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_shiftl"] = {
	...
	label = function(Out, A) return format("shiftL(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_shiftr"] = {
	...
	label = function(Out, A) return format("shiftR(%s) = (%d,%d,%d)", A, Out.x, Out.y, Out.z) end
}
...
GateActions["vector_isinworld"] = {
	...
	output = function(gate, A)
		...
		if util.IsInWorld(A) then
			return 1
		else
			return 0
		end
	end,
	label = function(Out, A) return format("isInWorld(%s) = %d", A, Out) end
}
...
GateActions["vector_rotate"] = {
	...
	label = function(Out, A, B) return format("rotate(%s, %s) = " .. tostring(Out), A, B) end
}
...
GateActions["vector_clampn"] = {
	...
	output = function(gate, A, Min, Max)
		...
		return Vector(Clamp(A.x, Min, Max), Clamp(A.y, Min, Max), Clamp(A.z, Min, Max))
	end,
...
GateActions["vector_clampv"] = {
	...
	output = function(gate, A, Min, Max)
		...
		return Vector(Clamp(A.x, Min.x, Max.x), Clamp(A.y, Min.y, Max.y), Clamp(A.z, Min.z, Max.z))
	end,
	label = function(Out, A, Min, Max) return "Clamp(" .. A .. "," .. Min .. "," .. Max .. ") = " .. tostring(Out) end
}
