...
GateActions["latch"] = {
	...
	label = function(Out, Data, Clk) return "Latch Data:" .. Data .. "  Clock:" .. Clk .. " = " .. Out end
}
...
GateActions["dlatch"] = {
	...
	label = function(Out, Data, Clk) return "D-Latch Data:" .. Data .. "  Clock:" .. Clk .. " = " .. Out end
}
...
GateActions["srlatch"] = {
	...
	label = function(Out, S, R) return "S:" .. S .. " R:" .. R .. " == " .. Out end
}
...
GateActions["toggle"] = {
	...
	label = function(Out, Clk, OnValue, OffValue) return "Off:" .. OffValue .. "  On:" .. OnValue .. "  Clock:" .. Clk .. " = " .. Out end
}
...
GateActions["wom4"] = {
	...
	output = function(gate, Clk, AddrWrite, Data)
		AddrWrite = floor(tonumber(AddrWrite))
	...
	label = function() return "Write Only Memory - 4 store" end
}
...
GateActions["ram8"] = {
	...
	output = function(gate, Clk, AddrRead, AddrWrite, Data, Reset)
		...
		AddrRead = floor(tonumber(AddrRead))
		AddrWrite = floor(tonumber(AddrWrite))
	...
	label = function(Out, Clk, AddrRead, AddrWrite, Data, Reset) return "WriteAddr:" .. AddrWrite .. "  Data:" .. Data .. "  Clock:" .. Clk .. "  Reset:" .. Reset .. "\nReadAddr:" .. AddrRead .. " = " .. Out end,
...
GateActions["ram128k"] = {
	...
	label = function(Out, Clk, AddrRead, AddrWrite, Data) return "WriteAddr:" .. AddrWrite .. "  Data:" .. Data .. "  Clock:" .. Clk .. "\nReadAddr:" .. AddrRead .. " = " .. Out end,
	ReadCell = function(dummy, gate, Address)
...
GateActions["ram64x64"] = {
	...
	output = function(gate, Clk, AddrReadX, AddrReadY, AddrWriteX, AddrWriteY, Data, Reset)
		...
		AddrReadX = floor(tonumber(AddrReadX))
		AddrReadY = floor(tonumber(AddrReadY))
		AddrWriteX = floor(tonumber(AddrWriteX))
		AddrWriteY = floor(tonumber(AddrWriteY))
		...
		if (AddrReadX < 0) or (AddrReadX >= 64) or (AddrReadY < 0) or (AddrReadY >= 64) then return 0 end
	...
	label = function(Out, Clk, AddrReadX, AddrReadY, AddrWriteX, AddrWriteY, Data, Reset) return "WriteAddr:" .. AddrWriteX .. ", " .. AddrWriteY .. "  Data:" .. Data .. "  Clock:" .. Clk .. "  Reset:" .. Reset .. "\nReadAddr:" .. AddrReadX .. ", " .. AddrReadY .. " = " .. Out end,
	ReadCell = function(dummy, gate, Address)
...
GateActions["udcounter"] = {
	...
	label = function(Out, Inc, Dec, Clk, Reset) return "Increment:" .. Inc .. " Decrement:" .. Dec .. " Clk:" .. Clk .. " Reset:" .. Reset .. " = " .. Out end
}
...
GateActions["togglewhile"] = {
	...
	label = function(Out, Clk, OnValue, OffValue, While) return "Off:" .. OffValue .. "  On:" .. OnValue .. "  Clock:" .. Clk .. "  While:" .. While .. " = " .. Out end
}
