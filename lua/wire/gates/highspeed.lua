...
GateActions["highspeed_write"] = {
	...
	label = function(Out, Clk, Memory, Address, Data) return format("Clock:%s Memory:%s Address:%s Data:%s = %s", Clk, Memory, Address, Data, Out) end
}
...
GateActions["highspeed_read"] = {
	...
	label = function(Out, Clk, Memory, Address) return format("Clock:%s Memory:%s Address:%s = %s", Clk, Memory, Address, Out) end
}
