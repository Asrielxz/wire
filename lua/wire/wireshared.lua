...
function pairs_sortvalues(tbl, criterion)
    local crit = criterion and function(a, b) return criterion(tbl[a], tbl[b]) end or function(a, b) return tbl[a] < tbl[b] end
    local tmp = {}
    tbl = tbl or {}
...
if SERVER then
    ...
    timer.Create("Debugger.PoolTypeStrings", 1, 1, function()
        if WireLib.Debugger and WireLib.Debugger.formatPort then
            -- Reduce bandwidth
            for typename, _ in pairs(WireLib.Debugger.formatPort) do
    ...
    -- To remove the inputs without to remove the entity. Very important for IsWire checks!
    function WireLib._RemoveWire(eid, DontSend)
...
elseif CLIENT then
    ...
    -- To remove the inputs without to remove the entity.
    function WireLib._RemoveWire(eid)
        ents_with_inputs[eid] = nil
        ents_with_outputs[eid] = nil
    end
...
-- nicetime
-------------------------
local times = {{"y", 31556926}, {"mon", 2629743.83}, {"w", 604800}, {"d", 86400}, {"h", 3600}, {"m", 60}, {"s", 1}} -- years -- months -- weeks -- days -- hours -- minutes -- seconds
...
function WireLib.GetClosestRealVehicle(vehicle, position, notify_this_player)
    ...
    if IsValid(notify_this_player) and notify_this_player:IsPlayer() and not IsRealVehicle(vehicle) then
        WireLib.AddNotify(notify_this_player, "The entity you linked to is not a 'real' vehicle, " .. "and we were unable to find any 'real' vehicles attached to it. This controller might not work.", NOTIFY_GENERIC, 14, NOTIFYSOUND_DRIP1)
    end
...
do
    ...
    local interestingBinds = {"invprev", "invnext", "impulse 100", "attack", "jump", "duck", "forward", "back", "use", "left", "right", "moveleft", "moveright", "attack2", "reload", "alt1", "alt2", "showscores", "speed", "walk", "zoom", "grenade1", "grenade2"}
