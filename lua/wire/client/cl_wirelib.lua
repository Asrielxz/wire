...
local function Wire_GetWireRenderBounds(ent)
        ...
        for net_name, wiretbl in pairs(ent.WirePaths) do
            ...
            for j = 1, len do
                ...
                if (node_ent:IsValid()) then
                    ...
                    if (nodepos.x < bbmin.x) then
                        bbmin.x = nodepos.x
                    end

                    if (nodepos.y < bbmin.y) then
                        bbmin.y = nodepos.y
                    end

                    if (nodepos.z < bbmin.z) then
                        bbmin.z = nodepos.z
                    end

                    if (nodepos.x > bbmax.x) then
                        bbmax.x = nodepos.x
                    end

                    if (nodepos.y > bbmax.y) then
                        bbmax.y = nodepos.y
                    end

                    if (nodepos.z > bbmax.z) then
                        bbmax.z = nodepos.z
                    end
                end
            end
    ...
    if (ent.ExtraRBoxPoints) then
        for _, point_l in pairs(ent.ExtraRBoxPoints) do
            ...
            if (point.x < bbmin.x) then
                bbmin.x = point.x
            end

            if (point.y < bbmin.y) then
                bbmin.y = point.y
            end

            if (point.z < bbmin.z) then
                bbmin.z = point.z
            end

            if (point.x > bbmax.x) then
                bbmax.x = point.x
            end

            if (point.y > bbmax.y) then
                bbmax.y = point.y
            end

            if (point.z > bbmax.z) then
                bbmax.z = point.z
            end
        end
    end
...
function WireLib.hud_debug(text, oneframe)
    hook.Add("HUDPaint", "wire_hud_debug", function()
        if oneframe then
            hook.Remove("HUDPaint", "wire_hud_debug")
        end
