...
function WireLib.TriggerInput(ent, name, value, ...)
    ...
    if not ok then
        ...
        if IsValid(ply) then
            WireLib.ClientError(message, ply)
        end
    end
end
...
function WireLib.TriggerOutput(ent, oname, value, iter)
    -- if WireLib.RateLimit:LImit() then
...
function WireLib.Link_Cancel(idx)
    ...
    if CurLink[idx].input then
        ...
        WireLib.Paths.Add({
            Entity = CurLink[idx].Dst,
            Name = CurLink[idx].DstId,
            Width = 0
        })
    end
...
-- class OutputIterator
duplicator.RegisterEntityModifier("WireDupeInfo", function(ply, Ent, DupeInfo) end) -- this does nothing for now, we need the blank function to get the duplicator to copy the WireDupeInfo into the pasted ent
...
function WireLib.ApplyDupeInfo(ply, ent, info, GetEntByID)
    ...
    if (info.Wires) then
        for k, input in pairs(info.Wires) do
            ...
            -- Input alias
            -- if the entity has any inputs and the input 'k' is not one of them...
            if ent.Inputs and not ent.Inputs[k] then
            ...
            if IsValid(ent2) then
                ...
                -- If the target entity has no wirelink output, create one (& more old dupe compatibility)
                if input.SrcId == "link" or input.SrcId == "wirelink" then
                    input.SrcId = "wirelink"

                    if not ent2.extended then
                        WireLib.CreateWirelinkOutput(ply, ent2, {true})
                    end
                elseif input.SrcId == "entity" and ((ent2.Outputs and not ent2.Outputs.entity) or not ent2.Outputs) then
                    -- if the input name is 'entity', and the target entity doesn't have that output...
                    WireLib.CreateEntityOutput(ply, ent2, {true})
                end

                -- Output alias
                -- if the target entity has any outputs and the output 'input.SrcId' is not one of them...
                if ent2.Outputs and not ent2.Outputs[input.SrcId] then
...
function getOwner(entity)
    ...
    if OnDieFunctions then
        if OnDieFunctions.GetCountUpdate and OnDieFunctions.GetCountUpdate.Args and OnDieFunctions.GetCountUpdate.Args[1] then return OnDieFunctions.GetCountUpdate.Args[1] end
        if OnDieFunctions.undo1 and OnDieFunctions.undo1.Args and OnDieFunctions.undo1.Args[2] then return OnDieFunctions.undo1.Args[2] end
    end

    if entity.CPPIGetOwner and IsValid(entity:CPPIGetOwner()) then
        owner = entity:CPPIGetOwner()
    elseif entity.GetOwner and IsValid(entity:GetOwner()) then
        owner = entity:GetOwner()
    elseif entity.GetPlayer and IsValid(entity:GetPlayer()) then
...
function WireLib.NumModelSkins(model)
    if NumModelSkins then return NumModelSkins(model) end
...
function WireLib.AddInputAlias(class, old, new)
    ...
    if not ENT_table.InputAliases then
        ENT_table.InputAliases = {}
    end
...
function WireLib.AddOutputAlias(class, old, new)
    ...
    if not ENT_table.OutputAliases then
        ENT_table.OutputAliases = {}
    end
