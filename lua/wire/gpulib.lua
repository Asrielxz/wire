...
if CLIENT then
    ...
    for i = 1, RT_CACHE_SIZE do
        local Target = {false, false} -- Is rendertarget in use -- The rendertarget (false if doesn't exist)
    ...
    local function GetRT()
        ...
        for i, RT in pairs(RenderTargetCache) do
            -- not used and doesn't exist, let's create the render target.
    ...
    function GPU:Finalize()
        ...
        -- This is to test if the entity has truly been removed. If you really know you need to remove the RT, call FreeRT()
        timer.Simple(0.2, function()
            if IsValid(self.Entity) then return end --MsgN(self,"Entity still exists, exiting.")
            self:FreeRT()
        end)
    end
    ...
    function GPU:Render(rotation, scale, width, height, postrenderfunction)
        ...
        local ok, err = xpcall(function()
            ...
            if postrenderfunction then
                postrenderfunction(pos, ang, res, aspect, monitor)
            end
        end, debug.traceback)
    ...
    function WireGPU_NeedRenderTarget(entindex)
        if not GPUs[entindex] then
            GPUs[entindex] = GPULib.WireGPU(Entity(entindex))
        end
...
if CLIENT then
    usermessage.Hook("wire_gpulib_setent", function(um)
        ...
        local x = -w / 2
        local y = -h / 2
        local corners = {{x, y}, {x, y + h}, {x + w, y}, {x + w, y + h}}
...
if CLIENT then
    ...
    local function GPULib_MemorySync(um)
        ...
        while true do
            ...
            if blockCount > 256 then
                error("GPULib usermessage read error")
            ...
            if offsetSize > 0 then
                ...
                if offsetSize == 1 then
                    deltaOffset = um:ReadChar()
                end

                if offsetSize == 2 then
                    deltaOffset = um:ReadShort()
                end

                if offsetSize == 3 then
                    deltaOffset = um:ReadFloat()
                end
...
elseif SERVER then
    ...
    function CACHEMGR:Flush(forcePlayer)
        ...
        if not self.ValueOrderMatters then
            table.sort(self.Cache, function(A, B) return A[1] < B[1] end)
        end
        ...
        for _, data in ipairs(self.Cache) do
            ...
            elseif sequentialBlock and (compressBlock.Size == size) then
                ...
                elseif compressBlock.Repeat > 1 then
                    ...
                    if not sequentialBlock then
                        compressBlock.SetOffset = address - previousBlockEnd
                    end
        ...
        for k, v in ipairs(compressInfo) do
            ...
            if v.SetOffset then
                ...
                if offsetSize == 2 then
                    dataFlags = dataFlags + 2
                end

                if offsetSize == 4 then
                    dataFlags = dataFlags + 3
                end
            end

            if v.Repeat > 1 then
                if v.Repeat == 2 then
                    dataFlags = dataFlags + 8
                elseif v.Repeat == 4 then
                    dataFlags = dataFlags + 12
                else
                    dataFlags = dataFlags + 4
                end
            end
            ...
            if v.Size == 2 then
                dataFlags = dataFlags + 64
            end

            if (v.Size == 4) and (not v.IsFloat) then
                dataFlags = dataFlags + 128
            end

            if (v.Size == 4) and (v.IsFloat) then
                dataFlags = dataFlags + 192
            end
            ...
            if v.SetOffset then
                ...
                if offsetSize == 1 then
                    umsg.Char(v.SetOffset)
                    messageSize = messageSize + 1
                end

                if offsetSize == 2 then
                    umsg.Short(v.SetOffset)
                    messageSize = messageSize + 2
                end

                if offsetSize == 4 then
                    umsg.Float(v.SetOffset)
                    messageSize = messageSize + 4
                end
            end
